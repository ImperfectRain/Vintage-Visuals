...
namespace VintageVisuals
{
    ...
    public class VintageVisualsModSystem : ModSystem
    {
        ...
        {
            if (group == PseudoPbrSubsystem.TerrainBaseGroupName)
            {
                return PseudoPbrSubsystem.TerrainBasePatchEnabled &&
            ...
            // Gated on whether ANY shader-side feature is asking for something,
            // not on the subsystem toggle. Height haze goes through the game's
            // ambient stack and no shader at all, so a player who wants only
            // that should get vanilla's own applyFog compiled rather than this
            // mod's arithmetic multiplied by nothing.
            //
            // That matters here beyond tidiness: with the group applied, this
            // mod owns applyFog in four programs.
            if (group == AtmosphereSubsystem.GroupName)
            {
                return ConfigManager.Config.Atmosphere.WantsShader;
            }
        ...
        private string PatchGatingSignature()
        {
            return (ConfigManager.Config.PseudoPBR.Enabled ? "P" : "-") +
                   ...
                   (ConfigManager.Config.Atmosphere.WantsShader ? "A" : "-");
        }
